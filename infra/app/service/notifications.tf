locals {
  ...
  dns_zone               = local.network_config.domain_config.hosted_zone
}
...
module "notifications_email_domain" {
  count  = module.app_config.enable_notifications && !local.is_temporary && local.dns_zone != null ? 1 : 0
...
module "notifications" {
  count  = module.app_config.enable_notifications && local.dns_zone != null ? 1 : 0
