...
* Provide an executable named `healthcheck` in container's `$PATH` which exits with code `0` if your service is healthy, and non-zero if not healthy. Or have `wget`, `curl`, or `bash` available, in which case the application's `/health` endpoint will be pinged for container healthchecks.
