...
In order to set up HTTPS support you'll also need to have [set up custom domains](/docs/infra/production-preparations/custom-domains.md). This is because SSL/TLS certificates must be properly configured for the specific domain to support establishing secure connections.
...
Update `enable_https = true` in your application's `app-config` module. You should have already set `domain_name` as part of [setting up custom domain names](/docs/infra/production-preparations/custom-domains.md).
